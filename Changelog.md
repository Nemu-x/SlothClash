...
**✨ JavaScript-переопределение конфига — то, чего не могут остальные редакторы**
- В профиль теперь можно положить `function main(config, ctx)`, которая переписывает сгенерированный конфиг до того, как его увидит ядро. Это для всего, что нельзя выразить редакторами extend-config / прокси-групп / правил: они декларативные, а тут — логика. Переименовать все ноды по маске, собрать группу на каждую страну, выкинуть ноды по имени, применить твик только в TUN-режиме. Открывается из контекстного меню профиля → **Скрипт (JS)**.
- **Превью до применения.** Превью прогоняет ваш скрипт через настоящий пайплайн генерации дважды — со скриптом и без — и показывает дифф двух конфигов бок о бок плюс всё, что вы вывели через `console.log`. Ничего не пишет и не трогает активное подключение, так что жать можно прямо на связи.
- **Сломанный скрипт не может сломать вам VPN.** Синтаксическая ошибка, исключение, бесконечный цикл, значение, которое нельзя записать в конфиг — результат скрипта отбрасывается, конфиг генерируется ровно так, как если бы скрипта не было, а причина (со строкой и колонкой) показывается в редакторе и бейджем на профиле. Подключение работает.
- **Скрипт не может отрезать приложение от собственного ядра.** Порт прокси, адрес контроллера с секретом и корпоративный VPN-сплит переутверждаются после вашего скрипта. Всё остальное — DNS, TUN, сниффер, правила, прокси, группы — ваше.
- Скрипты выполняются в песочнице без доступа к файлам, сети, процессам и таймерам, с чистым состоянием на каждый запуск, лимитом в 3 секунды и ограниченным буфером вывода. Скрипт может появиться только из вашего собственного редактора: подписка, заголовок провайдера, share-ссылка или импорт профиля выставить его не могут. Полное описание API: `docs/SCRIPT_OVERRIDE.md`.
