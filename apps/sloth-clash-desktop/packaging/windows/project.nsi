...
# Core SHA-256 pin set for the service re-pin on upgrade (generated by
# scripts/generate-core-pins.mjs into the same dir). /NONFATAL so a build that
# skipped generation still compiles; the re-pin block no-ops on an empty pin.
!include /NONFATAL "core_pins.nsh"
!ifndef SLOTH_CORE_SHA256_PINS
    !define SLOTH_CORE_SHA256_PINS ""
!endif
...
Section "${INFO_PRODUCTNAME}" SecApp
  ...
  ProcessesClosed:
    ...
    ; --- Service core re-pin (upgrade only) --------------------------------
    ; The privileged service only spawns a core whose SHA-256 matches a pin
    ; recorded in its environment (SLOTH_CLASH_CORE_SHA256). A core bump
    ; changes that hash, so an already-installed service would refuse the new
    ; core ("does not match any pinned hash") until reinstalled. Since this
    ; installer already runs elevated, refresh the pin in place on upgrade so
    ; users never hit that. Fresh installs have no service yet (the in-app
    ; "Install service" flow pins correctly) → the sc query guard skips them.
    ; NOTE: overwrites the whole Environment value; today it holds only this
    ; one var. If the service ever gains other env vars, extend this write.
    StrCmp "${SLOTH_CORE_SHA256_PINS}" "" RepinSkip 0
    nsExec::ExecToStack '"$SYSDIR\sc.exe" query sloth_clash_service'
    Pop $0
    Pop $1
    StrCmp $0 "0" 0 RepinSkip
    DetailPrint "Refreshing helper service core pin..."
